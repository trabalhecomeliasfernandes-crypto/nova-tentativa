...
export const generateMockData = (baseLeads: number, sqlRate: number, conversionRate: number): DailyRecord[] => {
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    data.push({
      ...
      sql: Math.max(0, sql),
      // Fix: The 'contratos' property does not exist on type 'DailyRecord'.
      // Replaced it with 'contratosFechados' and a calculated 'valorContratos' to match the type definition.
